...
public class OptimalPath {
	...
	public int calculateOptimalPath(int[][] grid) {
		...
		if (grid.length == 1) {
			int[] newArray = Arrays.copyOfRange(
					grid[0],
					1,
					grid[0].length
			);

			return total += calculateOptimalPath(
					new int[][] { newArray }
			);
		}
		...
		for (int i = grid.length - 1; i >= 0; i--) {
			alongPath[i] = Arrays.copyOfRange(
					grid[i],
					1,
					grid[0].length
			);
		...
		return total += Math.max(
				calculateOptimalPath(upPath),
				calculateOptimalPath(alongPath)
		);
	}
}
